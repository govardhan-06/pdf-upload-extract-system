    ...
    def process_page(self, page):
        ...
        if text_density < self.TEXT_DENSITY_THRESHOLD or word_count < self.MIN_WORD_COUNT:
            try:
                ...
                # Enqueue Dramatiq task and get message
                message = perform_ocr.send(
                    img_bytes,
                    page.number + 1,
                    page.rect.width,
                    page.rect.height
                ...
                self.redis_client.set(task_key, message.message_id, ex=3600)
            ...
            with fitz.open(stream=pdf_bytes, filetype="pdf") as doc:
                ...
                with ThreadPoolExecutor(max_workers=min(4, os.cpu_count())) as executor:
                    ...
                    for page_num, page_chunks in enumerate(results, start=start_page):
                        ...
                        message_id = self.redis_client.get(task_key)
                        if message_id:
                            ocr_messages.append(message_id)
                
                if ocr_messages:
                    for message_id in ocr_messages:
                        try:
                            message = dramatiq.Message(
                                queue_name="ocr_processing",
                                actor_name="perform_ocr",
                                args=(),
                                kwargs={},
                                options={},
                                message_id=message_id,
                            )
                            ocr_chunks = message.get_result(block=True, timeout=300000)  # 5 minutes timeout
                            if ocr_chunks:
                            ...
                            # Clean up Redis key
                            self.redis_client.delete(f"ocr_task:{message_id}")
